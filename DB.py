...
class DB():
    ...
    def write_db_Path(self,data): # Path에 해당하는 데이터를 Path 테이블에 기록한다 // data = [("",int,float,float,float), (,,,,)  .... ] 과 같은 형태 리스트 인덱스 순으로 기록된다. idx값이 같다면 새로운 데이터로 덮어쓴다
        for i,(x,y,yaw) in enumerate(data):
            self.__cur.execute(
                    "INSERT INTO Path (path_id,idx, x, y, yaw,speed) VALUES (?, ?, ?,?,?) ON CONFLICT(idx) DO UPDATE SET path_id = excluded.path_id, x=excluded.x, y=excluded.y, yaw=excluded.yaw",
                    (self.id, i, x, y, yaw,5),
                )
        self.__conn.commit()
    ...
    def find_idx(self,x,y,table):# idx ,id
        self.__cur.execute(f"SELECT id,value_x,value_y FROM {table}")
        rows = self.__cur.fetchall()
        print(rows)
        min_err  = 10000
        idx = 0
