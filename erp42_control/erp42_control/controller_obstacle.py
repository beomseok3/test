...
class PathHandler():
    def __init__(self, node):
        ...
        node.create_subscription(PoseArray, "final_roote", self.callback_path, qos_profile)
...
class Drive():
    def __init__(self, node, state, path):
        ...
        self.pub_target_speed_pub = node.create_publisher(Float32, "target_speed", qos_profile)
    ...
    def callback_speed(self,msg):
        ...
        data = Float32()
        self.path_speed = msg.data - 4.0
        data.data = self.path_speed
        self.pub_target_speed_pub.publish(data)
...
def main(args = None):
    ...
    # state = State(node, "odometry/kalman")
