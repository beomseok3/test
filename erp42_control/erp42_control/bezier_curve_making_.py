...
class BevierCurveMaking(Node):
    ...
    def point_select(self):
        ...
        if lenth > 0:
            ...
            self.publish_bezier(mid_bezier_x, mid_bezier_y, dx, dy)
    ...
    def publish_bezier(self, mid_bezier_x, mid_bezier_y, dx, dy):
        try:
            transform = self.tf_buffer.lookup_transform('odom', 'velodyne', self.get_clock().now().to_msg())
        except (tf2_ros.LookupException, tf2_ros.ConnectivityException, tf2_ros.ExtrapolationException):
            self.get_logger().warn("Failed to get transform from 'velodyne' to 'odom'")
            return
        ...
        for x, y, dx, dy in zip(mid_bezier_x, mid_bezier_y, dx, dy):
            ...
            path.poses.append(pose_transformed)
